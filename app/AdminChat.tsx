...
const AdminChat = () => {  
    ...
    const fetchMessages = async () => {
        try { 
            ...
            const data = response.data.notifications.reduce((acc, list) => {
                const senderId = list.sender._id;
                const existingMessage = acc.messages.find((msg) => msg.sender === senderId);

                if (!existingMessage || new Date(list.timestamp) > new Date(existingMessage.timestamp)) {
                    const messageData = {
                        id: list._id,
                        sender: senderId,
    ...
    const renderMessageItem = ({ item }: { item: Message }) => (  
        <TouchableOpacity
            onPress={() => {
                setUnreadCount(0);
                router.push({ pathname: "/ChatScreen", params: { name: item.fullName, receiver: item.sender } });
                ...
                setUnreadCount((prevCount) => prevCount - (item.status === "unread" ? 1 : 0));
            }}
        >
